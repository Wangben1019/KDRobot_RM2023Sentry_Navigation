...
private:
    ...
    PointCloudPtr new_corners_cloud_;   //点云指针
    cv::Mat img_mat_;                   //cv读图的一个Mat对象
    std::size_t img_counter_;           //TODO
    std::vector<CVPointStack> refined_contours_;    //存放contours_Point的一个栈
    std::vector<cv::Vec4i> refined_hierarchy_;      //TODO Vec4i 是一种用于表示 4 维 vector 的结构，每个值都是 int
    NavNodePtr odom_node_ptr_;          //里程计的一个节点指针，类型属于NavNode
