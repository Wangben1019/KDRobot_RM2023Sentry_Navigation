...
/*********************User Start*************************/
  input_ikfom user_in;
  void user_set_speed(input_ikfom &user_in)
  {
    this->user_in.acc = user_in.acc;
    this->user_in.gyro = user_in.gyro;
  }
  input_ikfom user_get_speed()
  {
    return this->user_in;
  }
/*********************User End***************************/
...
//代码中通过迭代的形式完成了IMU数据的更新，并将acc和gyro的数据传入到ESKF中
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  const double &pcl_end_time = meas.lidar_end_time; //拿到最后一帧时间戳加上最后一帧的所需要的时间/1000得到点云的结束时间戳
